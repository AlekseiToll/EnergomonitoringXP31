...
namespace EmGraphLib.Radial
{
	public class VectorCurveList
	{
		...
		public void _transform(VectorPair pair, double zeroAngle, double ratio, double nominal, Point p1, bool realValues, out Point p2)
		{
			...
			if (ratio == 0 && realValues)
			{
				pixModule = 0;
			}
			else
			{
				pixModule = realValues ? pair.Module / ratio : nominal / ratio;
			}

			double x = p1.X;
			double y = p1.Y;

			//Service.SDebug.Out("pixModule " + pixModule.ToString());
			...
			bool realValues, Color color, ref List<Point> newCurve)
		{
			try
			{
				...
				//Service.SDebug.Out(realValues.ToString() + "   " + p1.ToString() + "   " + p2.ToString());
			...
			{
				EmService.DumpException(ex, "Error in _drawPair(): ");
				throw;
			}
		}
			...
			foreach (VectorCurve curve in curves)
			{
				...
				for (int i = 0; i < curve.VectorPairList.Count; i++)
				{
					...
					if (i < curve.LegendsCount && i < styles.Length)
					{
						...
						//Service.SDebug.Out(curve.GetLegend(i).Label.Text);
						//Service.SDebug.Out(curve.VectorPairList[i].Module.ToString());
					}
				}
			}
			/////////////
