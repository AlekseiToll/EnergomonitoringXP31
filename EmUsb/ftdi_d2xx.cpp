...
{
		_asm rdtsc	// мультимедийный таймер __rdtsc();
}
//========================================================================
//========================================================================
...
функция должна прочитать заданное количество байт в ограниченное время
выход из функции происходит если заданное количество байт прочитано, либо если время ожидания истекло
...
// проверяем параметр на бредовое значение
	if ( dwTimeout == 0 ) dwTimeout = 100;
	if ( dwTimeout > MAX_TIMEOUT ) dwTimeout = MAX_TIMEOUT;

// вычисляем количество попыток чтения, укладывающееся во время ожидания
...
// устанавливаем минимальный таймаут на чтение
...
//		printf("(#)");
	...
	// если заданное число байт прочитано - выходим
		if ( dwReceived >= len )
		{
//			printf("( dwReceived >= len )", usStopCounter);
	...
	// возвращаем первоначальные установки
	SetTimeouts( MIN_TIMEOUT, dwWriteTimeoutMs);
	...
	LPBOOL bNormaOK )
{
	...
	*bNormaOK = false;
	...
	// открывем 
	// эта фуккция включает сброс, установку скорости, установку конфигурации (стоповые биты), установку таймаутов по умолчанию
			...
			LPDWORD dwReceivedLength)
{
	...
	sReceiveBuffer[1] = 0x00; // предустановка 
...
// попытки чтения
	...
	// appending newly received array to the previously received array
		...
		{
			break; // НЕ НОРМА - выходим чтобы не терять времени
...
Полный объем ФЛЭШ 64МБайта
Организована в виде 65536 страниц по 512 байт
Чтение без проверки CRC - режим 13
...
// preparing query message
	...
	// преамбула
	message[0] = 0x02;
	message[1] = 0x0D; // режим 13

	message[2] = 0x06;
	message[3] = 0x00; // число последующих байт

	message[4] = 0x00;
	message[5] = 0x00; // номер ФЛЭШ 0

	message[6] = (BYTE)(usFirstPage & 0xFF );
	message[7] = (BYTE)( usFirstPage >> 8 );	// номер начальной страницы

	message[8] = (BYTE)(usNumberPages & 0xFF );
	message[9] = (BYTE)( usNumberPages >> 8 ); // количество страниц
...
// preparing query message
	...
	// преамбула
	message[0] = 0x02;
	message[1] = 0x0F; // режим 15

	message[2] = 0x04;
	message[3] = 0x00; // число последующих байт

	message[4] = (BYTE)(usStartAddress & 0xFF );
	message[5] = (BYTE)( usStartAddress >> 8 );	// начальный адрес

	message[6] = (BYTE)(usNumberWords & 0xFF );
	message[7] = (BYTE)( usNumberWords >> 8 ); // количество слов
...
// preparing message
	...
	// преамбула
	message[0] = 0x02;
	message[1] = 0x0E; // режим 14

	message[2] = (BYTE)((usOutLength - 6) & 0xFF );
	message[3] = (BYTE)((usOutLength - 6) >> 8 ); // число последующих байт
...
// preparing message
	...
	// преамбула
	message[0] = 0x02;
	message[1] = 0x08; // режим 8

	message[2] = (BYTE)((usOutLength - 6) & 0xFF );
	message[3] = (BYTE)((usOutLength - 6) >> 8 ); // число последующих байт

	message[4] = (BYTE)( usPageNumber & 0xFF );
	message[5] = (BYTE)( usPageNumber >> 8 );	// номер страницы

	message[6] = (BYTE)( usOffset & 0xFF );
	message[7] = (BYTE)( usOffset >> 8 );	// смещение на странице

	message[8] = (BYTE)( usNumberWords & 0xFF );
	message[9] = (BYTE)( usNumberWords >> 8 ); // количество слов
