						 ...
						 long* arr_params, long params_count, long* pAnswerLen)
{
	try
	{
				...
				if (other_params.size() >= 2)
				{
					...
					switch (queryType)
					{
						...
						case AAQ_TYPE_Query:
							...
							for (int iParam = 15; iParam < other_params.size(); ++iParam)
							{
		...
		// получаем ответ
		bool bAllPacketsReceived = false, bTypeSenederIsCorrect = false;
		//BYTE* tempBuffer;
		...
		while (!bAllPacketsReceived)
		{
			...
			bTypeSenederIsCorrect = false;
			while (!bTypeSenederIsCorrect) // the cycle eliminates echo packets
			{
			...
			if (devType_ == ETPQP)
			{
				if ((command != actualReply_->wCommand) &&
					...
					!(command == COMMAND_ReadQualityEntryObjectDemand &&
			...
			// если это не был запрос усредненных, то сохраняем пакет и выходим
			if (command != COMMAND_AverageArchiveQuery &&
				actualReply_->wCommand != COMMAND_AverageArchiveQuery)
			{
				...
				{
					listTempBuffer.push_back(actualReply_->bData[iByte]);
				}
				break;
			}
			...
			// если запрашивались усредненные, но id запроса не совпадает с id ответа, то игнорируем
			// пакет
			...
			{
				std::stringstream ss;
				ss << "ReadData(): invalid AVG query type!  curType = " << curType << "  queryType = ";
				ss << queryType;
				EmService::WriteToLogFailed(ss.str());
				SetTimeoutCounter(timeOut);
				continue;
			...
			// если это была команда Reset для средних, то ждем немного, чтобы прибор успел среагировать
			if (queryType == 1 && curType == 1 /*ResetAll*/)
			{
				Sleep(1500);
			}

			// если запрашивались данные усредненных, то проверяем последний ли это сегмент
...
WORD EmPortSLIP::CalcCrc16(std::vector<BYTE>& buffer, int start, int len)
{
	...
	try
	{
		// последние два байта не рассматриваем, т.к. они зарезервированы для crc,
		// первый байт тоже пропускаем
		for (int i = start; i < (len + start - 2) && buffer.size() > len; ++i)
		{
...
void EmPortComSLIP::RxThread()
{
	...
	{
		EmService.EmService.WriteToLogFailed("ThreadAbortException in EmPortCom32.RxThread()");
		innerError_ = true;
	}*/
