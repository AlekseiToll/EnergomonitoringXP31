...
namespace NativeWifi
{
	...
	{
		public static string EtPqpAWifiProfile =
			"<?xml version=\"1.0\"?>\r\n<WLANProfile xmlns=\"http://www.microsoft.com/networking/WLAN/profile/v1\">\r\n\t<name>{0}</name>\r\n\t<SSIDConfig>\r\n\t\t<SSID>\r\n\t\t\t<hex>{2}</hex>\r\n\t\t\t<name>{0}</name>\r\n\t\t</SSID>\r\n\t</SSIDConfig>\r\n\t<connectionType>ESS</connectionType>\r\n\t<connectionMode>manual</connectionMode>\r\n\t<MSM>\r\n\t\t<security>\r\n\t\t\t<authEncryption>\r\n\t\t\t\t<authentication>WPA2PSK</authentication>\r\n\t\t\t\t<encryption>AES</encryption>\r\n\t\t\t\t<useOneX>false</useOneX>\r\n\t\t\t</authEncryption>\r\n\t\t\t<sharedKey>\r\n\t\t\t\t<keyType>passPhrase</keyType>\r\n\t\t\t\t<protected>false</protected>\r\n\t\t\t\t<keyMaterial>{1}</keyMaterial>\r\n\t\t\t</sharedKey>\r\n\t\t</security>\r\n\t</MSM>\r\n</WLANProfile>\r\n";

		public static bool IsWifiConnected(bool wait, WlanClient.WlanInterface wlanIface, string profileName)
		{
			try
			{
				if (wlanIface.InterfaceState == Wlan.WlanInterfaceState.Connected)
				{
					if (wlanIface.CurrentConnection.profileName.Contains(profileName))
						return true;
					return false;	// подключение есть, но не туда куда нам надо
				}
				else if (!wait) return false;
				else
				{
					EmService.WriteToLogDebug("Wi-fi Error 1: " + wlanIface.InterfaceName);
					EmService.WriteToLogDebug("Wi-fi Error 1: " + wlanIface.InterfaceDescription);
					EmService.WriteToLogDebug("Wi-fi Error 1: " + wlanIface.InterfaceState.ToString());
					Thread.Sleep(5000);
					EmService.WriteToLogDebug("Wi-fi Error 1.2: " + wlanIface.InterfaceState.ToString());
					if (wlanIface.InterfaceState == Wlan.WlanInterfaceState.Connected)
					{
						if (wlanIface.CurrentConnection.profileName.Contains(profileName))
							return true;
						else
							EmService.WriteToLogDebug("Wi-fi Error 1.2: " +
								wlanIface.CurrentConnection.profileName);
					}
					else
					{
						EmService.WriteToLogDebug("Wi-fi Error 2: " + wlanIface.InterfaceName);
				...
				EmService.WriteToLogDebug("Wi-fi Error 1.6: " + wlanIface.InterfaceState.ToString());
				return false;
			}
			catch (Exception ex)
			{
		...
		public static bool IsWifiConnected(bool wait, string profileName)
		{
			try
			{
				...
				foreach (WlanClient.WlanInterface wlanIface in client.Interfaces)
				{
					if (wlanIface.InterfaceState == Wlan.WlanInterfaceState.Connected)
					{
						if (wlanIface.CurrentConnection.profileName.Contains(profileName))
							return true;
						continue; // подключение есть, но не туда куда нам надо
					}
					else if (!wait) continue;
					else
					{
						EmService.WriteToLogDebug("Wi-fi Error 1: " + wlanIface.InterfaceName);
						EmService.WriteToLogDebug("Wi-fi Error 1: " + wlanIface.InterfaceDescription);
		...
		public static WlanClient.WlanInterface ConnectWifiEtpqpA(string profileName, string password)
		{
			...
			try
			{
				...
				foreach (WlanClient.WlanInterface wlanIface in client.Interfaces)
				{
					// delete previous profiles of the device
					Wlan.WlanProfileInfo[] profiles = wlanIface.GetProfiles();
					for (int iProf = 0; iProf < profiles.Length; ++iProf)
					{
						if (profiles[iProf].profileName.Contains(profileName))
							wlanIface.DeleteProfile(profiles[iProf].profileName);
					}

					// get profile name in ASCII symbols
					string profileNameHex = string.Empty;
					byte[] bhex = Encoding.ASCII.GetBytes(profileName);
					foreach (byte b in bhex)
					{
						profileNameHex += b.ToString("X");
					}

					// Connects to a network
					profileXml = string.Format(Wlan.EtPqpAWifiProfile, profileName, password, profileNameHex);
			...
			{
				EmService.DumpException(ex, "Exception in ConnectWifiEtpqpA():");
				EmService.WriteToLogFailed("////////////////////////////////////////////////////");
				EmService.WriteToLogFailed(profileXml);
				EmService.WriteToLogFailed("////////////////////////////////////////////////////");
				throw;
			}
		}
