...
namespace ZedGraph
{
	...
	/// <version> $Revision: 1.5 $ $Date: 2007/03/26 07:55:09 $ </version>
	...
	#region Properties
		...
		public bool IsShowHScrollBar
		{
			...
			set { isShowHScrollBar = value; ChangeSize( this, EventArgs.Empty ); }
		}
		/// <summary>
		...
		public bool IsShowVScrollBar
		{
			...
			set { isShowVScrollBar = value; ChangeSize( this, EventArgs.Empty ); }
		}
...
//				Cursor.Current = Cursors.No;
	...
	#region ContextMenu
		...
		private void ContextMenu_Popup( object sender, System.EventArgs e )
		{
			...
			if ( this.isShowContextMenu )
			{
				...
				menuItem = new MenuItem();
				menuItem.Index = index++;
				menuStr = resourceManager.GetString("cm_print", cultureInfo);
				menuItem.Text = menuStr;
				this.contextMenu.MenuItems.Add(menuItem);
				menuItem.Click += new EventHandler(this.MenuClick_Print);
				 */
				//if (pane == null)
				//	menuItem.Enabled = false;
		...
		protected void MenuClick_SaveAs( System.Object sender, System.EventArgs e )
		{
			...
			if (saveDlg.FilterIndex == 3)
				format = ImageFormat.Png;
			else
			if (saveDlg.FilterIndex == 4)
				format = ImageFormat.Tiff;
			else
			if (saveDlg.FilterIndex == 5)
			...
			if (myStream != null)
			{
				if (MasterPane.PaneRect.Height != 0 &&
					MasterPane.PaneRect.Width != 0)
				{
				...
				{
					//ResourceManager rm = 
					//new ResourceManager("ZedGraph.ZedGraphLocale", this.GetType().Assembly);
					//string msg = rm.GetString("confirm_cancel_msg");
					//string cap = rm.GetString("error");
					//MessageBox.Show(sender as Form, msg, cap, MessageBoxButtons.OK,
					//    MessageBoxIcon.Information);
					//this.MasterPane.GetImage(w, h).Save(myStream, format);
					MessageBox.Show(sender as Form, "„тобы сохранить изображение, необходимо закрепить панель графиков (запретить автоматическое скрытие)", "Error", MessageBoxButtons.OK,
						MessageBoxIcon.Information);
				}
				myStream.Close();
			}
		...
		//protected void MenuClick_Print(System.Object sender, System.EventArgs e)
		//{
		//	GraphPane pane = this.MasterPane.FindPane(this.menuClickPt);
		//	if (pane != null && !pane.zoomStack.IsEmpty)
		//	{
		...
		{
			this.MasterPane.sizePrint =
				new Size(e.PageBounds.Width, e.PageBounds.Height);
			this.MasterPane.bPrint = true;
			this.MasterPane.Draw(e.Graphics);
			this.MasterPane.bPrint = false;
			//Bitmap bmp = new Bitmap(this.MasterPane.Image.Width, this.MasterPane.Image.Height);
            //panel1.DrawToBitmap(bmp, new Rectangle(new Point(), bmp.Size));
            //e.Graphics.DrawImage(this.MasterPane.Image, e.PageBounds);
			//this.MasterPane.Print(e);
	...
	#region ScrollBars
		...
		public void SetScroll( ScrollBar scrollBar, Axis axis, double scrollMin, double scrollMax )
		{
			if ( scrollMin > axis.Min )
				scrollMin = axis.Min;
			if ( scrollMax < axis.Max )
