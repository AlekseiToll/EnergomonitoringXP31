...
namespace ZedGraph
{
	...
	/// <version> $Revision: 1.4 $ $Date: 2007/02/15 07:47:50 $ </version>
	[Serializable]
	public class Legend : ICloneable, ISerializable
	{
		...
		public void Draw( Graphics g, PaneBase pane, float scaleFactor )
		{
			...
			{
				List<string> labels = new List<string>();//??????????????mycode
				...
				foreach ( CurveItem curve in tmpPane.CurveList )
				{
					...
					{
						// if such label already exists ???????????mycode
						if (labels.Contains(curve.Label))
						{
							continue;
						}
