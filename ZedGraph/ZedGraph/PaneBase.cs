...
namespace ZedGraph
{
	...
	/// <version> $Revision: 1.4 $ $Date: 2007/02/15 07:47:52 $ </version>
	...
	#region Methods
		...
		/// </returns>
		/// <seealso cref="PkaneBase.BaseDimension"/>
		...
		public float CalcScaleFactor()
		{
			...
			{
				if (sizePrint.Height <= 0)
					return 1.0F;
			}

			float length = (bPrint != true) ? paneRect.Width : sizePrint.Width;
			float aspect = (bPrint != true) ? (paneRect.Width / paneRect.Height) :
				(sizePrint.Width / sizePrint.Height);

			if (aspect > ASPECTLIMIT)
				length = ((bPrint != true) ? paneRect.Height : sizePrint.Height) * ASPECTLIMIT;
			if (aspect < 1.0F / ASPECTLIMIT)
				length = ((bPrint != true) ? paneRect.Width : sizePrint.Width) * ASPECTLIMIT;
